...
interface UsersPaginationProps<TData> {
  table: Table<TData>
  pageCount: number
  page: number
...
export function UsersPagination<TData>({ table, pageCount, page, setPage }: UsersPaginationProps<TData>) {
  const queryClient = useQueryClient()

  const handlePageChange = (newPage: number) => {
    table.setPageIndex(newPage)
    setPage(newPage + 1)
    queryClient.invalidateQueries(['users', newPage + 1])
  }
